...
AUTHOR="Philipp Graf"
CONTACT="philipp.graf@t-systems.com"
DATE="22.11.2012"
VERSION="1.0.8"
#Version history
#1.0.0		phigraf		first version.
#1.0.1		phigraf		added warning output to INFO_FILE.
#						changed some ERRORS to WARNINGS
#						removed some checks from tivoli.
#1.0.2		phigraf		bugfixes.
#1.0.3		phigraf		added suppot for disabling single tivoli checks.
#1.0.4		phigraf		changed name to system_healthcheck_linux.sh.
#1.0.5		phigraf		added suppot for cciss device files.
#						fixed error handling for debug mode.
#1.0.6		Juraj Bujdoso	CHM0 - remove some check not important for appcom
#						CHM1 - check zombies
#						CHM2 -check IOwait, INODES
#						CHM3 - support info for appcom, hostname, vlan, customer, frame, img ,uptime
#						CHm15 inodes ussage
#1.0.7		phigraf		moved appcom check to fucntions.
#						added output for appcom infos in general.
#						added creation of csv file (/tmp/${HOSTNAME}-healtcheck.csv).
#						fixed some bugs.
#1.0.8		phigraf		added apccom cvs output to stdout. Delimiter is now ;
#
