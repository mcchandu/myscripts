...
#Declaring variables
#set -x
os_name=`uname -a | awk {'print$1 " "$2'} | cut -f2 -d'-'`
...
os_version=`uname -r | cut -f2 -d'~' | awk {'print$1'} | cut -f1 -d'-' | cut -c 1-5`
num_users=`w | head -1 | awk '{print$4}'`
cpu_free=`top b -n1 | head -5 | head -3 | tail -1 | awk '{print$8}' | cut -f1 -d ','`
last_reboot=`who -b | awk '{print$3, $4}'`
df -h | awk '{print$6, $5}'| grep -v Mounted | cut -f1 -d '%' > /tmp/fsstat.txt
...
#email_add="chandra.reddy@jktech.com"
EMAIL='chandra.reddy@jktech.com,sajan.purayil@jktech.com'
...
function drawPieChart() {
  ...
  echo "]);" >> $html
  echo -e "var options = {title:'Overall Filesystem Usage',
  width:600,
  height:400,
  is3D: true
};
...
#echo "<h3><legend>Script authored by chandra</legend></h3>" >> $html
echo "</center>" >> $html
echo "</fieldset>" >> $html
echo "<br>" >> $html
echo "<center>" >> $html
...
echo "<br>" >> $html
echo "<div class=\"panel panel-info\" style=\"width: 40%;\"> " >> $html
...
echo "</div> " >> $html
echo "</div> " >> $html
echo -e "<div class=\"panel panel-info\" style=\"width: 50%;\">
...
mailx -a $html -s "qaddev Linux Server Report" $EMAIL  < /dev/null
